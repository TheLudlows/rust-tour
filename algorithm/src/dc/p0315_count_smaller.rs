...
impl Solution {
    pub fn count_smaller(mut nums: Vec<i32>) -> Vec<i32> {
        ...
        mergeSort(&mut nums_copy, 0, nums_copy.len(), &mut count);
        count
    }
}

fn mergeSort(nums: &mut Vec<i32>, left: usize, right: usizem, count: &mut Vec<i32>) {
    if left >= right {
        return
    }
...
fn merge(nums: &mut Vec<i32>, left: usize, right: usizem, count: &mut Vec<i32>) {
    let mid = (left + right) / 2;
    let mut tmp = vec![];
